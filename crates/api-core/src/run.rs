...
pub async fn run(
    ...
    // its [credentials.*] section is enabled. The backend stores (postgres,
    ...
    // `stores`/`writer` -- defaulting to env -> file -> vault writing to vault,
    ...
    let (credential_manager, secrets_context): (
        ...
        tracing::info!(
            ...
            stores = ?secrets_config.stores,
            writer = ?secrets_config.writer,
            "Postgres secrets backend configured"
        );
        // New writes all go to `writer`, but reads take the first store in
        // `stores` that holds the path -- first-match-wins, evaluated per path.
        // So unless `writer` is the highest-priority store, a write can be
        // shadowed: if a higher-priority store also holds that path, reads keep
        // returning its value and never reach the writer's. E.g. with
        // stores = [vault, postgres] and writer = postgres, a path that exists
        ...
        // is not in `stores` at all. Both reduce to "writer isn't the top
        // store." We allow it -- a deliberate shadow-write is a valid, if
        // advanced, setup -- but warn so an accidental one is visible.
        let writer_is_top_store = secrets_config.stores.first() == Some(&secrets_config.writer);
        if !writer_is_top_store {
            tracing::warn!(
                writer = ?secrets_config.writer,
                stores = ?secrets_config.stores,
                "secrets writer's backend is not the highest-priority store: a write to a path a \
                 higher-priority store also holds is shadowed on read until that copy is removed \
                 (read-after-write gap)"
            );
        }

        // A one-time bulk import from vault, only when the operator asks for
        // one. Independent of stores/writer.
        ...
        // stores in the operator's chosen order (first match wins). The write
        // target is the single store `writer` names.
        crate::secrets::validate_stores(&secrets_config.stores)?;
        let store_readers =
            secrets_config
                .stores
                .iter()
                .map(|store| -> Box<dyn CredentialReader> {
                    match store {
        ...
        let chain: Vec<Box<dyn CredentialReader>> =
            local_overrides.into_iter().chain(store_readers).collect();
...
/// exactly as `stores` / `writer` set them -- importing changes neither.
///
