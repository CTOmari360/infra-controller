...
func (mh *APIMachineHealth) FromProto(protoHealth *cwssaws.HealthReport) {
	...
	mh.ObservedAt = cutil.GetPtr(protoHealth.ObservedAt.AsTime().Format(time.RFC3339))
	mh.ObservedAtDeprecated = cutil.GetPtr(protoHealth.ObservedAt.AsTime().Format(time.RFC3339))
...
func (ahpa *APIMachineHealthProbeAlert) FromProto(protoAlert *cwssaws.HealthProbeAlert) {
	...
	ahpa.InAlertSince = cutil.GetPtr(protoAlert.InAlertSince.AsTime().Format(time.RFC3339))
	ahpa.InAlertSinceDeprecated = cutil.GetPtr(protoAlert.InAlertSince.AsTime().Format(time.RFC3339))
