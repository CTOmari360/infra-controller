...
// GetDpuMachinesHandler is the API Handler for retrieving DPU machines attached to a host Machine.
type GetDpuMachinesHandler struct {
	dbSession  *cdb.Session
	scp        *sc.ClientPool
	tracerSpan *cutil.TracerSpan
}

// NewGetDpuMachinesHandler initializes and returns a new handler to retrieve Machine DPU machines.
func NewGetDpuMachinesHandler(dbSession *cdb.Session, scp *sc.ClientPool) GetDpuMachinesHandler {
	return GetDpuMachinesHandler{
		dbSession:  dbSession,
		scp:        scp,
		tracerSpan: cutil.NewTracerSpan(),
	}
...
func (gmdh GetDpuMachinesHandler) Handle(c echo.Context) error {
	org, dbUser, ctx, logger, handlerSpan := common.SetupHandler("Machine", "GetDpu", c, gmdh.tracerSpan)
	if handlerSpan != nil {
		defer handlerSpan.End()
	}
	if dbUser == nil {
	...
	mID := c.Param("id")
	gmdh.tracerSpan.SetAttribute(handlerSpan, attribute.String("machine_id", mID), logger)

	// Get Machine with Site relation
	mDAO := cdbm.NewMachineDAO(gmdh.dbSession)
	machine, err := mDAO.GetByID(ctx, nil, mID, []string{cdbm.SiteRelationName}, false)
	if err != nil {
		if err == cdb.ErrDoesNotExist {
			return cutil.NewAPIErrorResponse(c, http.StatusNotFound, "Could not find Machine with specified ID", nil)
	...
	provider, tenant, apiError := common.IsProviderOrTenant(ctx, logger, gmdh.dbSession, org, dbUser, false, true)
	if apiError != nil {
		return cutil.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
	...
	if provider != nil {
		...
		taDAO := cdbm.NewTenantAccountDAO(gmdh.dbSession)
		_, taCount, serr := taDAO.GetAll(ctx, nil, cdbm.TenantAccountFilterInput{
	...
	stc, err := gmdh.scp.GetClientByID(machine.SiteID)
	if err != nil {
		logger.Error().Err(err).Msg("failed to retrieve Temporal client for Site")
	...
	miDAO := cdbm.NewMachineInterfaceDAO(gmdh.dbSession)
	...
	apiDpuMachines := model.APIDpuMachines{}
	...
	// Return empty response if no DPUs are attached
	...
	apiDpuMachines.FromResponseProto(controllerDpuMachines, model.APIDpuMachineProtoContext{
		HostMachineID:            mID,
		SiteID:                   site.ID,
		InfrastructureProviderID: site.InfrastructureProviderID,
	})
